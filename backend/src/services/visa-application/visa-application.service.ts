...
const visaApplicationService = {
  async listAll(userId: string, user: any) {
  ...
  async deleteMany(applicationIds: string[]) {
    const removed = await visaApplicationRepo.deleteMany(applicationIds);
    if (!removed) throw new Error("Failed to delete Applications");
